...
<p align="center">
  ...
  <a href="file:///home/commander/docs/Getting_Started.md">Docs Hub</a> |
  <a href="file:///home/commander/docs/SUMMARY.md">Docs TOC</a>
</p>

<p align="center">
  <strong>Quick Routes:</strong>
  <a href="file:///home/commander/docs/Architecture.md">Architecture</a> ·
  <a href="file:///home/commander/docs/Configuration.md">Config</a> ·
  <a href="file:///home/commander/docs/Operations_Guide.md">Operations</a> ·
  <a href="file:///home/commander/docs/Security_Protocol.md">Security</a> ·
  <a href="file:///home/commander/docs/getting-started/README.md">Setup</a> ·
  <a href="file:///home/commander/docs/contributing/README.md">Contribute</a>
</p>
...
vector_weight = 0.7
keyword_weight = 0.3
cpu_idle_threshold_percent = 90  # CPU idle threshold for scheduling memory hygiene (0-100)
max_wait_hours = 24              # Maximum hours to wait for CPU idle before forcing hygiene
...
ZeroClaw includes advanced security and stability features to ensure safe, reliable operation in production environments.

#### Process Management & Resource Isolation

- **Process Group Support**: All shell commands are executed in dedicated process groups (PGIDs), enabling clean termination of entire process trees including child processes
- **Workspace Isolation**: Each hand (agent instance) operates in an isolated workspace at `/tmp/zeroclaw/hands/{hand_id}`, preventing file conflicts between concurrent operations
- **Automatic Cleanup**: Workspaces are automatically cleaned up when hands complete, ensuring no temporary files are left behind
- **Interruption Handling**: Graceful termination of all processes when operations are interrupted, preventing orphaned processes

#### CPU-Aware Resource Management

- **CPU Monitoring**: Built-in CPU usage monitoring to detect system load and prevent resource exhaustion
- **Idle Threshold Configuration**: Configurable CPU idle threshold (0-100) determines when the system has sufficient resources for background operations
- **Smart Scheduling**: Memory hygiene and maintenance operations wait for CPU idle periods before running, minimizing impact on active workloads
- **Resource-Aware Behavior**: Operations adapt to current system load, maintaining responsiveness even under heavy utilization

**CPU Idle Threshold Configuration:**

```toml
[resource_management]
cpu_idle_threshold = 80  # CPU is idle when usage < 20%
```

The CPU monitor considers the system "idle enough" when global CPU usage is below `(100 - threshold)` percent. For example, with `cpu_idle_threshold = 80`, the system is idle when CPU usage drops below 20%.

#### Firewall Security (UFW Integration)

- **Automated Firewall Setup**: UFW (Uncomplicated Firewall) configuration script included for secure deployments
- **Default Deny Policy**: All incoming connections denied by default, with explicit allow rules for required services
- **Rate Limiting**: Built-in rate limiting for the ZeroClaw API (port 42617) to prevent brute force attacks
- **SSH Protection**: Automatic SSH access preservation to prevent lockout during firewall configuration
- **Bootstrap Integration**: Firewall configuration automatically runs during system bootstrap (can be skipped with `SKIP_FIREWALL=1`)
...
**UFW Configuration Features:**
- Deny all incoming traffic by default
- Allow all outgoing traffic
- Preserve SSH access (port 22)
- Rate-limit ZeroClaw API (port 42617)
- Allow localhost access (127.0.0.1 and ::1)
- Enable medium-level logging for security monitoring

#### Web Persistence (Frontend)

- **LocalStorage Integration**: User preferences and session data persisted using localStorage with `zeroclaw_` prefix
- **IndexedDB Storage**: Large datasets (messages, conversation history) stored in IndexedDB for efficient retrieval and offline support
- **Automatic Reconnection**: A2A (Agent-to-Agent) stream hooks include reconnection logic for resilient network communication
- **State Synchronization**: Initial state fetch hooks ensure consistent UI state across page reloads
- **Dual Storage Strategy**: Small config data in localStorage for fast access, large message data in IndexedDB for capacity

**Storage Architecture:**

```typescript
// LocalStorage: Fast access for small config items
localStorage.setItem('zeroclaw_config', JSON.stringify(config));

// IndexedDB: Large-scale message storage
const db = await openDB('zeroclaw-db', 1);
