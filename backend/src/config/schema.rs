...
// ModelProviderConfig is now in llm_schema.rs
// use crate::config::schemas::llm_schema::ModelProviderConfig;

// ── Delegate Agents ──────────────────────────────────────────────
// DelegateAgentConfig is now in llm_schema.rs
// use crate::config::schemas::llm_schema::DelegateAgentConfig;
...
/// Swarm configuration for sub-agent coordination (`[agent.swarm]` section).
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct SwarmConfig {
    /// Automatically spawn sub-agents for parallel task execution. Default: `false`.
    #[serde(default)]
    pub auto_spawn: bool,
    /// Maximum number of concurrent sub-agent hands. Default: `10`.
    #[serde(default = "default_swarm_max_active_hands")]
    pub max_active_hands: usize,
    /// Maximum time-to-live for a sub-agent hand in seconds. Default: `300` (5 minutes).
    #[serde(default = "default_swarm_max_hand_ttl_seconds")]
    pub max_hand_ttl_seconds: u64,
}

fn default_swarm_max_active_hands() -> usize {
    10
}

fn default_swarm_max_hand_ttl_seconds() -> u64 {
    300
}
...
impl Default for SwarmConfig {
    fn default() -> Self {
        Self {
            auto_spawn: false,
            max_active_hands: default_swarm_max_active_hands(),
            max_hand_ttl_seconds: default_swarm_max_hand_ttl_seconds(),
        }
    }
}
...
pub struct AgentConfig {
    ...
    /// Maximum context tokens to prevent exceeding model limits. Default: `200000`.
    /// Messages are trimmed when exceeding this limit (10% safety margin applied).
    ...
    /// Swarm configuration for sub-agent coordination.
...
// ── Memory ───────────────────────────────────────────────────
// Memory types are now in memory_schema.rs
// use crate::config::schemas::memory_schema::{
...
pub struct ReliabilityConfig {
    ...
    /// Provider-specific model aliases. Maps provider names to model name translations.
...
impl Default for ReliabilityConfig {
    fn default() -> Self {
        Self {
            ...
            provider_model_aliases: std::collections::HashMap::new(),
...
// ── Model routing ────────────────────────────────────────────────
// LLM routing types are now in llm_schema.rs
// use crate::config::schemas::llm_schema::{
//     ModelRouteConfig, EmbeddingRouteConfig,
//     QueryClassificationConfig, ClassificationRule,
// };
...
"#;
    ...
    async fn memory_config_default_hygiene_settings() {
        ...
        assert_eq!(m.cpu_idle_threshold_percent, 90);
        assert_eq!(m.max_wait_hours, 24);
    }

    #[test]
    async fn memory_config_custom_cpu_values() {
        let toml = r#"
            backend = "sqlite"
            auto_save = true
            ...
            max_wait_hours = 48
        "#;
        let m: MemoryConfig = toml::from_str(toml).expect("custom CPU values should parse");
        assert_eq!(m.cpu_idle_threshold_percent, 80);
        assert_eq!(m.max_wait_hours, 48);
    }
