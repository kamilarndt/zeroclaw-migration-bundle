...
pub struct MemoryConfig {
    ...
    // ── CPU-aware scheduling options ────────────────────────────
    /// CPU idle threshold percentage (0-100) for scheduling memory hygiene.
    /// Hygiene tasks will only run when CPU idle time is above this threshold.
    /// Default: 90
    #[serde(default = "default_cpu_idle_threshold")]
    pub cpu_idle_threshold_percent: u8,
    /// Maximum hours to wait for CPU idle threshold before forcing hygiene.
    /// If CPU stays busy beyond this time, hygiene runs anyway.
    /// Default: 24
...
fn default_embedding_model() -> String {
    // Multilingual model for better Polish language support
    // Alternative for Ollama users: "mxbai-embed-large"
...
impl Default for MemoryConfig {
    fn default() -> Self {
        Self {
            ...
            cpu_idle_threshold_percent: default_cpu_idle_threshold(),
