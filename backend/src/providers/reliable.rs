...
fn is_non_retryable_rate_limit(err: &anyhow::Error) -> bool {
    ...
    for token in lower.split(|c: char| !c.is_ascii_digit()) {
        if let Ok(code) = token.parse::<u16>() {
            // Z.AI: 1113 (insufficient quota), 1310 (rate limit), 1311 (quota exhausted)
            if matches!(code, 1113 | 1310 | 1311) {
                return true;
            }
        }
    }
...
pub struct ReliableProvider {
    ...
    /// Provider-specific model aliases: provider_name → {model_name → alias}
    provider_model_aliases: HashMap<String, HashMap<String, String>>,
}
...
impl ReliableProvider {
    pub fn new(
        ...
        Self {
            ...
            provider_model_aliases: HashMap::new(),
        }
    }
    ...
    /// Set provider-specific model aliases.
    /// When a provider doesn't support a model, translate it to an alias.
    /// Example: ollama provider may map "glm-4.7" -> "codellama:7b"
    pub fn with_provider_model_aliases(
        mut self,
        aliases: HashMap<String, HashMap<String, String>>,
    ) -> Self {
        self.provider_model_aliases = aliases;
        self
    }

    /// Translate model name for a specific provider using configured aliases.
    fn translate_model_for_provider(&self, provider_name: &str, model: &str) -> String {
        if let Some(aliases) = self.provider_model_aliases.get(provider_name) {
            if let Some(alias) = aliases.get(model) {
                tracing::debug!(
                    provider = provider_name,
                    original_model = model,
                    alias_model = alias,
                    "Translating model for provider"
                );
                return alias.clone();
            }
        }
...
impl Provider for ReliableProvider {
        ...
        for current_model in &models {
            for (provider_name, provider) in &self.providers {
                ...
                for attempt in 0..=self.max_retries {
                    match provider
                        .chat_with_system(system_prompt, message, &translated_model, temperature)
                        .await
                    {
                        Ok(resp) => {
    ...
    async fn chat_with_history(
        ...
        for current_model in &models {
            for (provider_name, provider) in &self.providers {
                ...
                for attempt in 0..=self.max_retries {
                    match provider
                        .chat_with_history(messages, &translated_model, temperature)
                        .await
                    {
                        Ok(resp) => {
                            if attempt > 0 || *current_model != model {
    ...
    async fn chat_with_tools(
        ...
        for current_model in &models {
            for (provider_name, provider) in &self.providers {
                ...
                for attempt in 0..=self.max_retries {
                    match provider
                        .chat_with_tools(messages, tools, &translated_model, temperature)
                        .await
                    {
                        Ok(resp) => {
                            if attempt > 0 || *current_model != model {
        ...
        for current_model in &models {
            for (provider_name, provider) in &self.providers {
                ...
                for attempt in 0..=self.max_retries {
                    ...
                    match provider.chat(req, &translated_model, temperature).await {
                        Ok(resp) => {
                            if attempt > 0 || *current_model != model {
                                tracing::info!(
                                    provider = provider_name,
    ...
    fn stream_chat_with_system(
        ...
        for (provider_name, provider) in &self.providers {
            ...
            let original_model = match self.model_chain(model).first() {
                Some(m) => m.to_string(),
