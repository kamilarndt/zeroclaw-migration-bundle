...
impl GlmProvider {
    ...
    async fn generate_token(&self) -> anyhow::Result<String> {
        ...
        // Wrap CPU-heavy crypto operations in spawn_blocking to prevent Tokio starvation
        let api_key_secret = self.api_key_secret.clone();
        let (sig_b64, signing_input) = tokio::task::spawn_blocking(move || {
...
mod tests {
    ...
    #[tokio::test]
    async fn generates_jwt_token() {
        let p = GlmProvider::new(Some("testid.testsecret"));
        let token = p.generate_token().await.unwrap();
    ...
    async fn caches_token() {
        let p = GlmProvider::new(Some("testid.testsecret"));
        let token1 = p.generate_token().await.unwrap();
        let token2 = p.generate_token().await.unwrap();
    ...
    async fn fails_without_key() {
        let p = GlmProvider::new(None);
        let result = p.generate_token().await;
