...
impl EstopManager {
    ...
    async fn ensure_resume_is_authorized(
        &self,
        otp_code: Option<&str>,
        otp_validator: Option<&OtpValidator>,
    ) -> Result<()> {
        ...
        let valid = validator.validate(code).await?;
        if !valid {
...
mod tests {
    ...
    #[tokio::test]
    async fn estop_levels_compose_and_resume() {
    ...
    #[tokio::test]
    async fn resume_requires_valid_otp_when_enabled() {
        let dir = tempdir().unwrap();
        let state_path = dir.path().join("estop-state.json");
    ...
    #[tokio::test]
    async fn resume_accepts_valid_otp_code() {
        let dir = tempdir().unwrap();
        let state_path = dir.path().join("estop-state.json");
        let mut cfg = estop_config(&state_path);
        cfg.require_otp_to_resume = true;
