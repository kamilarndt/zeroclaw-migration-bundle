...
impl OtpValidator {
    ...
    pub async fn validate(&self, code: &str) -> Result<bool> {
        self.validate_at(code, unix_timestamp_now()).await
    }

    async fn validate_at(&self, code: &str, now_secs: u64) -> Result<bool> {
        ...
        let is_valid = tokio::task::spawn_blocking(move || {
            counters
                .iter()
                .map(|c| compute_totp_code(&secret_clone, *c))
                .any(|candidate| candidate == normalized_owned)
        })
        .await
        .unwrap_or(false);
...
mod tests {
    ...
    #[tokio::test]
    async fn valid_totp_code_is_accepted() {
        ...
        assert!(validator.validate_at(&code, now).await.unwrap());
    }

    #[tokio::test]
    async fn expired_totp_code_is_rejected() {
        let dir = tempdir().unwrap();
        let store = SecretStore::new(dir.path(), true);
        let (validator, _) = OtpValidator::from_config(&test_config(), dir.path(), &store).unwrap();
    ...
    #[tokio::test]
    async fn secret_is_generated_and_reused() {
        let dir = tempdir().unwrap();
        let store = SecretStore::new(dir.path(), true);
