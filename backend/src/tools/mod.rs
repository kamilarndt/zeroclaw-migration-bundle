...
// pub use registry::{create_global_registry, DefaultToolRegistry, ToolFactory, ToolMetadata, ToolRegistry};
// pub use schema_builder::{SchemaBuilder, SimpleSchema};
...
pub fn all_tools_with_runtime(
    ...
    runtime: Arc<dyn RuntimeAdapter>,
    memory: Arc<dyn Memory>,
    hands: Option<Arc<crate::agent::hands::HandsDispatcher>>,
    composio_key: Option<&str>,
    composio_entity_id: Option<&str>,
    browser_config: &crate::config::BrowserConfig,
    http_config: &crate::config::HttpRequestConfig,
    ...
    let mut shell_tool = ShellTool::new(security.clone(), runtime);
    if let Some(ref h) = hands {
        shell_tool = shell_tool.with_dispatcher(h.clone());
    }
    ...
    // Add task planning tool when using SQLite memory backend
    ...
    // Subagent spawn tool for swarm coordination
    let mut subagent_tool = SubagentSpawnTool::new();
    if let Some(h) = hands {
        subagent_tool = subagent_tool.with_hands_dispatcher(h);
    }
    tool_arcs.push(Arc::new(subagent_tool));
    ...
    if !agents.is_empty() {
        let delegate_agents: HashMap<String, DelegateAgentConfig> = agents
            .iter()
            .map(|(name, cfg): (&String, &DelegateAgentConfig)| (name.clone(), cfg.clone()))
            .collect();
