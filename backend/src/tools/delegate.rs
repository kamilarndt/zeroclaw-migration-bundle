    ...
    fn parameters_schema(&self) -> serde_json::Value {
        ...
        json!({
            ...
            "properties": {
                ...
                "run_in_background": {
                    "type": "boolean",
                    "description": "If true, run the delegated task in the background (non-blocking)"
                },
            },
...
impl DelegateTool {
    async fn execute_agentic(
        ...
        let sub_tools: Vec<Box<dyn Tool>> = self
            .parent_tools
            .iter()
            .filter(|tool: &&Arc<dyn Tool>| allowed.contains(tool.name()))
