...
pub async fn run(config: Config, host: String, port: u16) -> Result<()> {
    ...
    if config.memory.hygiene_enabled {
        let hygiene_cfg = config.clone();
        handles.push(spawn_component_supervisor(
            "hygiene",
            initial_backoff,
            max_backoff,
            move || {
                let cfg = hygiene_cfg.clone();
                async move { Box::pin(run_hygiene_worker(cfg)).await }
            },
        ));
    } else {
        crate::health::mark_component_ok("hygiene");
        tracing::info!("Memory hygiene disabled; hygiene supervisor not started");
    }

    println!("🧠 ZeroClaw daemon started");
    println!("   Gateway:  http://{host}:{port}");
    println!("   Components: gateway, channels, heartbeat, scheduler, hygiene");
...
async fn run_hygiene_worker(config: Config) -> Result<()> {
    let workspace_dir = config.workspace_dir.clone();
    // Check hourly; internal logic determines if actual work is needed
    ...
    loop {
        ...
        match crate::memory::hygiene::run_if_due_cpu_aware(
            ...
            Err(e) => {
                crate::health::mark_component_error("hygiene", e.to_string());
...
mod tests {
    ...
    fn detects_dingtalk_as_supervised_channel() {
        let mut config = Config::default();
        config.channels_config.dingtalk = Some(crate::config::DingTalkConfig {
    ...
    fn detects_qq_as_supervised_channel() {
        let mut config = Config::default();
        config.channels_config.qq = Some(crate::config::QQConfig {
            app_id: "app-id".into(),
    ...
    fn detects_nextcloud_talk_as_supervised_channel() {
        let mut config = Config::default();
        config.channels_config.nextcloud_talk = Some(crate::config::NextcloudTalkConfig {
