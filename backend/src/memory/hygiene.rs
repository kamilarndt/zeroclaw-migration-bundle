...
/// Run memory/session hygiene if the cadence window has elapsed, with CPU-aware scheduling.
///
/// This async function checks if CPU is idle enough before running hygiene.
/// If CPU is busy, it will wait up to `config.max_wait_hours` for CPU to become idle.
/// If CPU never reaches idle threshold, hygiene is skipped.
///
/// This function is intentionally best-effort: callers should log and continue on failure.
pub async fn run_if_due_cpu_aware(config: &MemoryConfig, workspace_dir: &Path) -> Result<()> {
    ...
    // Create CPU monitor with configured threshold
    let mut monitor = CpuMonitor::new(config.cpu_idle_threshold_percent);

    // Check if CPU is idle enough
    if !monitor.is_idle_enough() {
        tracing::info!(
            "CPU busy, waiting for idle window... (threshold: {}%, max wait: {}h)",
            config.cpu_idle_threshold_percent,
            config.max_wait_hours
        );

        let max_wait = Duration::from_secs(config.max_wait_hours * 3600);
        let became_idle = monitor.wait_until_idle(max_wait).await;

        if !became_idle {
            tracing::info!(
                "CPU never reached idle threshold within {}h, skipping hygiene",
                config.max_wait_hours
            );
            return Ok(());
        }

        tracing::info!("CPU reached idle threshold, proceeding with hygiene");
    }

    // Run the actual hygiene work (reuse existing logic)
    let report = HygieneReport {
        archived_memory_files: archive_daily_memory_files(
            workspace_dir,
            config.archive_after_days,
...
fn should_run_now(workspace_dir: &Path) -> Result<bool> {
    ...
    Ok(Utc::now().signed_duration_since(last) >= chrono::Duration::hours(HYGIENE_INTERVAL_HOURS))
}
...
fn archive_daily_memory_files(workspace_dir: &Path, archive_after_days: u32) -> Result<u64> {
    ...
    let cutoff = Local::now().date_naive() - chrono::Duration::days(i64::from(archive_after_days));
    let mut moved = 0_u64;
...
fn purge_memory_archives(workspace_dir: &Path, purge_after_days: u32) -> Result<u64> {
    ...
    let cutoff = Local::now().date_naive() - chrono::Duration::days(i64::from(purge_after_days));
...
mod tests {
    ...
    fn skips_second_run_within_cadence_window() {
        ...
        let old_a = (Local::now().date_naive() - chrono::Duration::days(10))
            .format("%Y-%m-%d")
            .to_string();
        ...
        let old_b = (Local::now().date_naive() - chrono::Duration::days(9))
            .format("%Y-%m-%d")
            .to_string();
    ...
    fn purges_old_memory_archives() {
        ...
        let old = (Local::now().date_naive() - chrono::Duration::days(40))
            .format("%Y-%m-%d")
            .to_string();
        let keep = (Local::now().date_naive() - chrono::Duration::days(5))
            .format("%Y-%m-%d")
            .to_string();
    ...
    async fn prunes_old_conversation_rows_in_sqlite_backend() {
        ...
        let old_cutoff = (Local::now() - chrono::Duration::days(60)).to_rfc3339();
        conn.execute(
    ...
    #[tokio::test]
    async fn cpu_aware_hygiene_runs_when_cpu_idle() {
        let tmp = TempDir::new().unwrap();
        let workspace = tmp.path();
        fs::create_dir_all(workspace.join("memory")).unwrap();
        ...
        let mut cfg = default_cfg();
        cfg.cpu_idle_threshold_percent = 100; // Always idle (usage < 0% is impossible, but this tests the logic)
        cfg.max_wait_hours = 1;

        // Should run immediately since CPU check passes (or times out quickly with threshold 100)
    ...
    #[tokio::test]
    async fn cpu_aware_hygiene_skips_when_cpu_never_idle() {
        let tmp = TempDir::new().unwrap();
        let workspace = tmp.path();
        fs::create_dir_all(workspace.join("memory")).unwrap();
        ...
        let mut cfg = default_cfg();
        cfg.cpu_idle_threshold_percent = 0; // Never idle (usage < 100% is never true)
        cfg.max_wait_hours = 0; // No wait, immediate skip

        // Should skip since CPU never becomes idle
        run_if_due_cpu_aware(&cfg, workspace).await.unwrap();
...
/// Proactive memory hygiene loop that classifies unclassified memory records
pub async fn hygiene_loop(
    _db: Arc<dyn super::Memory>,
    _ollama: Arc<crate::providers::ollama::OllamaProvider>,
    shutdown: CancellationToken,
) -> anyhow::Result<()> {
    loop {
        tokio::select! {
            _ = sleep(Duration::from_secs(3600u64)) => {
            ...
            _ = shutdown.cancelled() => {
                tracing::info!("Memory hygiene loop shutting down");
                break;
            }
        }
    }
