...
impl SqliteMemory {
    ...
    pub fn with_embedder(
        ...
        // busy_timeout: wait up to 5 seconds for locked database
    ...
    fn init_schema(conn: &Connection) -> anyhow::Result<()> {
        ...
        // Migration: add agent_tasks table if not present
    ...
    async fn get_or_compute_embedding(&self, text: &str) -> anyhow::Result<Option<Vec<f32>>> {
        ...
        let cached = tokio::task::spawn_blocking(move || -> anyhow::Result<Option<Vec<f32>>> {
            let hash = Self::content_hash(&text_owned);
            let conn = conn.lock();
            let mut stmt =
        ...
        // Clone embedding to avoid lifetime issues in spawn_blocking
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            let hash = Self::content_hash(&text_owned2);
            let bytes = vector::vec_to_bytes(&embedding_owned);
            let conn = conn.lock();
            conn.execute(
                "INSERT OR REPLACE INTO embedding_cache (content_hash, embedding, created_at, accessed_at)
                 VALUES (?1, ?2, ?3, ?4)",
                params![hash, bytes, now_clone, now_clone],
            )?;
            conn.execute(
    ...
    pub async fn get_chat_history(
        ...
        let category_str = Self::category_to_str(&MemoryCategory::Conversation);
    ...
    /// Count total chat messages for a specific session
    pub async fn count_chat_messages(&self, session_id: &str) -> anyhow::Result<i64> {
        let conn = self.conn.clone();
        let session_id = session_id.to_string();
        let category_str = Self::category_to_str(&MemoryCategory::Conversation);
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<i64> {
            ...
            let count: i64 = conn.query_row(
                "SELECT COUNT(*) FROM memories WHERE session_id = ?1 AND category = ?2",
                params![session_id, category_str],
                |row| row.get(0),
            )?;
