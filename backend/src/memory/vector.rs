...
/// Async version of cosine similarity — runs computation on a blocking thread
/// to prevent Tokio starvation during CPU-intensive vector operations.
pub async fn cosine_similarity_async(a: Vec<f32>, b: Vec<f32>) -> f32 {
    tokio::task::spawn_blocking(move || cosine_similarity(&a, &b))
        .await
        .unwrap_or(0.0)
}
...
/// Async version of vec_to_bytes
pub async fn vec_to_bytes_async(v: Vec<f32>) -> Vec<u8> {
    tokio::task::spawn_blocking(move || vec_to_bytes(&v))
        .await
        .unwrap_or_else(|_| vec![])
}

/// Async version of bytes_to_vec
pub async fn bytes_to_vec_async(bytes: Vec<u8>) -> Vec<f32> {
    tokio::task::spawn_blocking(move || bytes_to_vec(&bytes))
        .await
        .unwrap_or_else(|_| vec![])
}
