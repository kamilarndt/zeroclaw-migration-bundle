        ...
        if jwt::verify_token(token, &state.jwt_secret).is_err() {
            ...
            if !state.pairing.is_authenticated(token).await {
                return (
                    ...
                    "Unauthorized — provide valid ?token=<jwt_or_bearer_token>",
                )
                    .into_response();
            }
        }
...
async fn handle_socket(socket: WebSocket, state: AppState) {
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                if sender.send(Message::Ping(Bytes::copy_from_slice(&[0u8]))).await.is_err() {
                    break;
                }
            }
            msg = receiver.next() => {
                let msg = match msg {
                    ...
                    Some(Ok(Message::Pong(_))) => continue, // Heartbeat response
                    Some(Err(e)) => {
                ...
                tracing::info!("💬 WebSocket Chat: {}", content);
