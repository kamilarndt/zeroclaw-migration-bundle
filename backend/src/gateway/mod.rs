...
// Channel types removed - using simplified channels
use crate::config::Config;
use crate::cost::CostTracker;
use crate::memory::{self, Memory, MemoryCategory, embeddings::EmbeddingProvider};
...
pub struct AppState {
    ...
    // Channel fields removed - channels module simplified
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let hands = Arc::new(crate::agent::hands::HandsDispatcher::default());
    ...
    // ── Skills Engine v2.0 (optional, requires Qdrant) ─────────────
    let (skill_engine, skill_loader, skill_evaluator) = {
        use crate::memory::embeddings::create_embedding_provider;
        use crate::memory::qdrant::QdrantMemory;

        // Get Qdrant config - check if URL is configured
        let qdrant_url = config.memory.qdrant.url
            .clone()
            .or_else(|| std::env::var("QDRANT_URL").ok())
            .unwrap_or_else(|| "http://localhost:6333".to_string());

        // Create embedder for SkillsEngine
        ...
        // Skip if embedder has 0 dimensions (noop embedder)
        if embedder.dimensions() == 0 {
            tracing::info!("SkillsEngine disabled: embedding provider has 0 dimensions");
            (None, None, None)
        } else {
            // Create dedicated QdrantMemory for skills with "skills_index" collection
            let qdrant_api_key = config.memory.qdrant.api_key.clone()
                .or_else(|| std::env::var("QDRANT_API_KEY").ok());

            match QdrantMemory::new(
                &qdrant_url,
                "skills_index", // Dedicated collection for skills
                qdrant_api_key,
                embedder.clone(),
            ).await {
                Ok(qdrant_mem) => {
                    let qdrant_arc = Arc::new(qdrant_mem);

                    // Initialize SkillsEngine with the same embedder
                    match SkillsEngine::new(&config.workspace_dir, qdrant_arc, embedder) {
                        Ok(engine) => {
                            let engine_arc = Arc::new(engine);

                            // Ensure the engine is initialized
                            if let Err(e) = engine_arc.ensure_initialized().await {
                                tracing::error!("SkillsEngine initialization failed: {e}");
                                (None, None, None)
                            } else {
                                tracing::info!("✅ SkillsEngine v2.0 initialized with Qdrant at {}", &qdrant_url);

                                // Create loader for skill matching
                                let loader = Arc::new(VectorSkillLoader::new(engine_arc.clone(), 0.75));

                                // Create evaluator with Ollama fallback for skill validation
                        ...
                        Err(e) => {
                            tracing::error!("Failed to create SkillsEngine: {e}");
                            (None, None, None)
                        }
                    }
                }
                Err(e) => {
                    tracing::warn!("SkillsEngine disabled: Qdrant connection failed at {}: {}", qdrant_url, e);
                    (None, None, None)
                }
            }
        }
    ...
    // Webhook secret removed - webhook channel deleted
    ...
    let config_put_router = Router::new()
        ...
        .route("/api/v1/config", put(api::handle_api_config_put))
    ...
    let app = Router::new()
        ...
        .route("/api/pair", post(handle_pair)) // ← ADDED: Frontend compatibility
        .route("/api/v1/pair", post(handle_pair)) // ← ADDED: Frontend API v1 compatibility
        .route("/v1/pair", post(handle_pair)) // ← ADDED: Caddy strips /api prefix
        .route("/webhook", post(handle_webhook))
        // ── Web Dashboard API routes ──
        .route("/api/status", get(api::handle_api_status))
        .route("/api/v1/status", get(api::handle_api_status)) // ← DODANE: /api/v1/status
        .route("/api/config", get(api::handle_api_config_get))
        .route("/api/v1/config", get(api::handle_api_v1_config_get))
        ...
        .route("/api/tasks/{id}", put(api::handle_api_tasks_update))
        .route("/api/tasks/{id}", delete(api::handle_api_tasks_delete))
        // ── Tasks API v1 ──
        .route("/api/v1/tasks", get(api::handle_api_tasks_list))
        ...
        // ── SOPs API ──
        .route("/v1/sops", get(api::handle_api_sops_list))
        .route("/v1/sops", post(api::handle_api_sops_create))
        .route("/v1/sops/{id}", put(api::handle_api_sops_update))
        // NOTE: DELETE endpoint not implemented - Memory trait lacks delete method
        // ── SSE event stream ──
        ...
        .with_state(state)
        // ── CORS: explicit configuration for proper preflight handling ──
        .layer(
            CorsLayer::new()
                .allow_origin(Any)
...
async fn run_gateway_chat_simple(state: &AppState, message: &str) -> anyhow::Result<String> {
    ...
    let (workspace_dir, identity_config) = {
        let config_guard = state.config.lock();
        (config_guard.workspace_dir.clone(), config_guard.identity.clone())
    };

    let tools: Vec<(&str, &str)> = state.tools_registry.iter()
        .map(|spec| (spec.name.as_str(), spec.description.as_str()))
        .collect();

    let skills = if let Some(skill_engine) = &state.skill_engine {
        skill_engine.list_skills(true).await?
    } else {
        vec![]
    };
...
/// Full-featured chat with tools for webhook handlers.
...
mod tests {
    ...
    fn webhook_memory_key_is_unique() {
        ...
        assert_ne!(key1, key2, "each call should generate a unique key");
    }
