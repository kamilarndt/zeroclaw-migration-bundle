...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "${SKIP_FIREWALL:-}" != "1" ]]; then
    ...
    if [[ -f "$SCRIPT_DIR/configure_ufw.sh" ]]; then
      if bash "$SCRIPT_DIR/configure_ufw.sh"; then
        info "UFW firewall configured successfully"
      else
        warn "UFW firewall configuration failed, but continuing..."
        warn "You can configure it later with: sudo ./scripts/configure_ufw.sh"
      fi
    else
      warn "UFW configuration script not found at $SCRIPT_DIR/configure_ufw.sh"
    fi
  else
    info "Skipping firewall configuration (SKIP_FIREWALL=1)"
  fi
fi
